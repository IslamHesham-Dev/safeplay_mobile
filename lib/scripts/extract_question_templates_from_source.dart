...
/// Extract all question templates from source code and export to JSON
/// This script reads the data directly from the seeding scripts without needing Firebase
class QuestionTemplateExtractor {
  /// Get all templates from source code
  Map<String, dynamic> getAllTemplates() {
    final allTemplates = <Map<String, dynamic>>[];

    // Junior Explorer Math Templates
    allTemplates.addAll(_getJuniorExplorerMathTemplates());

    // Junior Explorer English Templates
    allTemplates.addAll(_getJuniorExplorerEnglishTemplates());

    // Bright Minds Math Templates
    allTemplates.addAll(_getBrightMindsMathTemplates());

    // Bright Minds English Templates
    allTemplates.addAll(_getBrightMindsEnglishTemplates());

    // Mindful Exercise Templates
    allTemplates.addAll(_getMindfulExerciseTemplates());

    // Convert to JSON-serializable format
    final jsonTemplates = allTemplates.map((template) {
      return _convertToJson(template);
    }).toList();

    return {
      'metadata': {
        'exportDate': DateTime.now().toIso8601String(),
        'totalTemplates': jsonTemplates.length,
  ...
  /// Convert template map to JSON-serializable format
  Map<String, dynamic> _convertToJson(Map<String, dynamic> template) {
    final json = <String, dynamic>{
      'title': template['title'],
      'type': _enumToString(template['type']),
    ...
    if (template['explanation'] != null) {
      json['explanation'] = template['explanation'];
    }

    if (template['hint'] != null) {
      json['hint'] = template['hint'];
    }

    if (template['ageGroups'] != null) {
      json['ageGroups'] =
          (template['ageGroups'] as List).map((e) => _enumToString(e)).toList();
    }

    if (template['subjects'] != null) {
      json['subjects'] =
          (template['subjects'] as List).map((e) => _enumToString(e)).toList();
    }

    if (template['gameTypes'] != null) {
      json['gameTypes'] =
          (template['gameTypes'] as List).map((e) => _enumToString(e)).toList();
    }

    if (template['difficulty'] != null) {
      json['difficulty'] = _enumToString(template['difficulty']);
    }

    if (template['duration'] != null) {
      json['duration'] = template['duration'];
    }
  ...
  String _enumToString(dynamic enumValue) {
    ...
    if (enumValue is QuestionType) {
      return enumValue.rawValue;
    } else if (enumValue is AgeGroup) {
      return enumValue.name;
    } else if (enumValue is ActivitySubject) {
      return enumValue.name;
    } else if (enumValue is GameType) {
      return enumValue.name;
    } else if (enumValue is Difficulty) {
      return enumValue.name;
    }
  ...
  /// Junior Explorer (6-8) Mathematics templates
  List<Map<String, dynamic>> _getJuniorExplorerMathTemplates() {
    return [
  ...
  /// Junior Explorer (6-8) English Language Arts templates
  List<Map<String, dynamic>> _getJuniorExplorerEnglishTemplates() {
    return [
  ...
  /// Bright Minds (9-12) Mathematics templates
  List<Map<String, dynamic>> _getBrightMindsMathTemplates() {
    return [
  ...
  /// Bright Minds (9-12) English Language Arts templates
  List<Map<String, dynamic>> _getBrightMindsEnglishTemplates() {
    return [
      // Memory Match Templates
      {
  ...
  /// Mindful Exercise templates for both age groups
  List<Map<String, dynamic>> _getMindfulExerciseTemplates() {
    return [
...
/// Export function that returns JSON string
String exportQuestionTemplatesToJson() {
  final extractor = QuestionTemplateExtractor();
  final data = extractor.getAllTemplates();
