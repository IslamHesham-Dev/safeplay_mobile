...
/// Math Adventures games category using Firestore question templates
class MathAdventuresGamesScreen extends StatefulWidget {
  const MathAdventuresGamesScreen({super.key});

  @override
  State<MathAdventuresGamesScreen> createState() =>
      _MathAdventuresGamesScreenState();
}

class _MathAdventuresGamesScreenState extends State<MathAdventuresGamesScreen>
    with TickerProviderStateMixin {
  late final AnimationController _animationController;
  late final AnimationController _mathController;
  late final Animation<double> _fadeAnimation;

  int _currentGameIndex = 0;
  int _totalCoins = 0;
  List<QuestionTemplate> _mathTemplates = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadMathTemplates() async {
    try {
      final service = QuestionTemplateService();
      final templates = await service.getTemplatesByAgeAndSubject(
        ageGroup: AgeGroup.junior,
        subject: ActivitySubject.math,
      );

      setState(() {
        _mathTemplates = templates.take(5).toList(); // Take first 5 for games
        _loading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget _buildGameContent() {
    if (_currentGameIndex >= _mathTemplates.length) {
      return _buildCompletionScreen();
    }

    final template = _mathTemplates[_currentGameIndex];
  ...
  Widget _buildItemContent(String text, bool isVisible) {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            isVisible ? JuniorTheme.primaryBlue : JuniorTheme.backgroundLight,
        borderRadius: BorderRadius.circular(JuniorTheme.radiusMedium),
        border: Border.all(
          color: JuniorTheme.primaryBlue,
          width: 2,
  ...
  Widget _buildAnswerInput() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: JuniorTheme.backgroundLight,
        borderRadius: BorderRadius.circular(JuniorTheme.radiusMedium),
        border: Border.all(color: JuniorTheme.primaryBlue),
      ),
      child: TextField(
        textAlign: TextAlign.center,
        style: JuniorTheme.headingMedium,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: JuniorTheme.spacingLarge),
            Text(
              'You completed all Math Adventures!',
              style: JuniorTheme.bodyLarge,
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: JuniorTheme.spacingLarge),
  ...
  void _showSuccessFeedback() {
    setState(() {
      _totalCoins += _mathTemplates[_currentGameIndex].defaultPoints;
    });

    // Start math animation
    ...
    showDialog(
      ...
      builder: (context) => JuniorCelebrationOverlay(
        ...
        message: 'Excellent!',
        subMessage:
            'You earned ${_mathTemplates[_currentGameIndex].defaultPoints} coins!',
        points: _mathTemplates[_currentGameIndex].defaultPoints,
        onDismiss: () {
          Navigator.of(context).pop(); // Close confetti
          setState(() {
            _currentGameIndex++;
            _mathController.reset();
          });
        },
      ),
    );
