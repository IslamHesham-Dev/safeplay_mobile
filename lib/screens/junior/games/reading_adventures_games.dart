...
/// Reading Adventures games category using Firestore question templates
class ReadingAdventuresGamesScreen extends StatefulWidget {
  const ReadingAdventuresGamesScreen({super.key});

  @override
  State<ReadingAdventuresGamesScreen> createState() =>
      _ReadingAdventuresGamesScreenState();
}

class _ReadingAdventuresGamesScreenState
    extends State<ReadingAdventuresGamesScreen> with TickerProviderStateMixin {
  late final AnimationController _animationController;
  late final AnimationController _readingController;
  late final Animation<double> _fadeAnimation;

  int _currentGameIndex = 0;
  int _totalCoins = 0;
  List<QuestionTemplate> _readingTemplates = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadReadingTemplates() async {
    try {
  ...
  Widget _buildGameContent() {
    if (_currentGameIndex >= _readingTemplates.length) {
      return _buildCompletionScreen();
    }

    final template = _readingTemplates[_currentGameIndex];
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: JuniorTheme.primaryPurple.withOpacity(0.2),
                    borderRadius:
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(JuniorTheme.radiusMedium),
            border: Border.all(color: JuniorTheme.primaryPurple, width: 2),
          ),
  ...
  Widget _buildItemContent(String text, bool isVisible) {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            isVisible ? JuniorTheme.primaryPurple : JuniorTheme.backgroundLight,
        borderRadius: BorderRadius.circular(JuniorTheme.radiusMedium),
        border: Border.all(
          color: JuniorTheme.primaryPurple,
          width: 2,
  ...
  Widget _buildAnswerInput() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: JuniorTheme.backgroundLight,
        borderRadius: BorderRadius.circular(JuniorTheme.radiusMedium),
        border: Border.all(color: JuniorTheme.primaryPurple),
      ),
      child: TextField(
        textAlign: TextAlign.center,
        style: JuniorTheme.headingMedium,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: JuniorTheme.spacingLarge),
            Text(
              'You completed all Reading Adventures!',
              style: JuniorTheme.bodyLarge,
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: JuniorTheme.spacingLarge),
  ...
  void _showSuccessFeedback() {
    setState(() {
      _totalCoins += _readingTemplates[_currentGameIndex].defaultPoints;
    });
    ...
    showDialog(
      ...
      builder: (context) => JuniorCelebrationOverlay(
        ...
        message: 'Wonderful!',
        subMessage:
            'You earned ${_readingTemplates[_currentGameIndex].defaultPoints} coins!',
        points: _readingTemplates[_currentGameIndex].defaultPoints,
        onDismiss: () {
          Navigator.of(context).pop(); // Close confetti
          setState(() {
            _currentGameIndex++;
            _readingController.reset();
          });
        },
      ),
    );
