...
import 'question_template_service.dart';

/// Database initializer that runs automatically to ensure all required data is available
class DatabaseInitializer {
  ...
  Future<void> initializeDatabase() async {
    ...
    try {
      ...
      final existingTemplates = await _checkExistingTemplates();

      if (existingTemplates.isEmpty) {
        debugPrint('📚 No templates found, seeding curriculum templates...');
        await _seedAllCurriculumTemplates();
      } else {
        debugPrint('✅ Found ${existingTemplates.length} existing templates');
      }

      // Set up database indexes for performance
      await _setupDatabaseIndexes();
  ...
  Future<List<QuestionTemplate>> _checkExistingTemplates() async {
    try {
      ...
      debugPrint('Error checking existing templates: $e');
      return [];
    }
  }

  /// Seed all curriculum-aligned templates
  Future<void> _seedAllCurriculumTemplates() async {
    debugPrint('🌱 Seeding curriculum templates...');
    ...
    debugPrint('✅ All curriculum templates seeded successfully!');
  }
  ...
  Future<void> _seedJuniorExplorerMathTemplates() async {
    ...
    for (final template in templates) {
      try {
        ...
        debugPrint(
            '❌ Error creating Junior Math template ${template['title']}: $e');
      }
    }

    debugPrint('✅ Seeded ${templates.length} Junior Explorer Math templates');
  }
  ...
  Future<void> _seedJuniorExplorerEnglishTemplates() async {
    ...
    for (final template in templates) {
      try {
        ...
        debugPrint(
            '❌ Error creating Junior English template ${template['title']}: $e');
      }
    }

    debugPrint(
        '✅ Seeded ${templates.length} Junior Explorer English templates');
  }
  ...
  Future<void> _seedBrightMindsMathTemplates() async {
    ...
    for (final template in templates) {
      try {
        ...
        debugPrint(
            '❌ Error creating Bright Math template ${template['title']}: $e');
      }
    }

    debugPrint('✅ Seeded ${templates.length} Bright Minds Math templates');
  }
  ...
  Future<void> _seedBrightMindsEnglishTemplates() async {
    ...
    for (final template in templates) {
      try {
        ...
        debugPrint(
            '❌ Error creating Bright English template ${template['title']}: $e');
      }
    }

    debugPrint('✅ Seeded ${templates.length} Bright Minds English templates');
  }
  ...
  Future<void> _seedMindfulExerciseTemplates() async {
    ...
    for (final template in templates) {
      try {
        ...
        debugPrint(
            '❌ Error creating Mindful template ${template['title']}: $e');
      }
    }

    debugPrint('✅ Seeded ${templates.length} Mindful Exercise templates');
  }

  /// Set up database indexes for optimal performance
  Future<void> _setupDatabaseIndexes() async {
    debugPrint('📊 Setting up database indexes...');

    // Create indexes for common queries
    try {
      // Templates collection indexes
      await _firestore.collection('questionTemplates').doc('_indexes').set({
        'isActive': true,
        'subjects': true,
        'ageGroups': true,
        'skills': true,
      ...
      await _firestore.collection('activities').doc('_indexes').set({
        'teacherId': true,
        'publishState': true,
        'ageGroup': true,
        'subject': true,
      ...
      // Child submissions collection indexes
      await _firestore.collection('childSubmissions').doc('_indexes').set({
        'childId': true,
        'activityId': true,
        'submittedAt': true,
        'score': true,
      ...
      debugPrint('✅ Database indexes configured');
    } catch (e) {
      debugPrint(
          '⚠️ Note: Index creation may require manual setup in Firebase Console');
    }
  }

  /// Set up visibility rules for content publishing
  Future<void> _setupVisibilityRules() async {
    debugPrint('🔒 Setting up visibility rules...');
    ...
    debugPrint('✅ Visibility rules configured');
  }
}
